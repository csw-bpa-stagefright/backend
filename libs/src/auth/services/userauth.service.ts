...
export class UserAuthService {
    ...
    private async AdminUserExists(
        email: string
      ): Promise<UserExistReturnObject> {
        try {
          const usersQuery = await this.prisma.adminUser.findFirst({
            where: {
              email: email
            }
          })
    ...
    async VerifyAdminUserWithCredentials(
        email: string,
        password: string
      ): Promise<VerifyUserReturnObject> {
        try {
          const verifyUserExists: UserExistReturnObject = await this.AdminUserExists(email as string);

          if (verifyUserExists.statusCode === 1) {
            return { statusCode: 1, errorInfo: "Internal Server Error: [ADMINUSER_AUTH_SERVICE] Failed", errorCode: 500 }
          }
